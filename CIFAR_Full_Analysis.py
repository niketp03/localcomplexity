...
def get_config():
  ...
  config.train_batch_size = 256 #256
  ...
  config.wandb_proj = 'LRLC_study_CIFAR_MLP'
  config.wandb_pref = 'CIFAR-MLP'

  config.resume_step = 0

  ## mlp params
  config.input_dim = 3072  # 3x32x32 for CIFAR
  config.output_dim = 10
  config.hidden_dim = 1000
  config.n_layers = 7
  config.input_weights_init_scale = 0.3 #np.sqrt(2) #sets initialization standard deviation to be = (input_weights_init_scale)/sqrt(input_dim) 
  config.output_weights_init_scale = 0.3 #np.sqrt(2)
  ...
  config.lr_schedule_type = 'cosine'  # options: 'step', 'cosine', 'linear'
...
train_dataset = torchvision.datasets.CIFAR10('../cifar_data',
                                           download=True,
                                           train=True,
...
def train(model, loaders, config):
    ...
    if config.use_lr_schedule:
        if config.lr_schedule_type == 'step':
            scheduler = lr_scheduler.StepLR(opt, 
            ...
            raise ValueError(f"Unknown lr schedule type: {config.lr_schedule_type}")
        ...
        if config.lr_warmup_steps > 0:
            scheduler = lr_scheduler.SequentialLR(
                opt,
                [
                    lr_scheduler.LinearLR(opt, start_factor=0.1, end_factor=1.0, total_iters=config.lr_warmup_steps),
                    scheduler
                ],
...
# init wandb
wandb.init(project="cifar-analysis", config=config)

# Create model based on config
def create_model(config):
    model = MLP(
        input_dim=3072,  # CIFAR images are 3x32x32 = 3072 pixels
        hidden_dim=config.hidden_dim,
        output_dim=10,  # 10 classes for CIFAR
